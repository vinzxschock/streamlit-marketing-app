...
def generate_prompt_bild(available_data):
    ...
    prompt_bild += "Die Zielgruppeninformationen folgen in dem Format von Personas oder Zielgruppen. Es kann eine oder mehrere Personas/Zielgruppen geben. Deine Aufgabe ist es die Instanzen zu einem Gesamtbild der Zielgruppe(n) zusammenzufassen.\n\n"
...
def generate_prompt_text(available_data):
    ...
    if available_data['A_Logo']:
        prompt_text += (
            f"Erstelle eine Launchkampagne für ein Unternehmen mit den folgenden Unternehmensinformationen, Zielgruppeninformationen und Auftragsinformationen."
            f"\n\n"
            f"  <unternehmensinformationen>"
            f"\n\n"
            f"      <unternehmensname>{st.session_state['U_Daten']['unternehmensname']}</unternehmensname>"
    ...
    if available_data['A_Produkt']:
        prompt_text +=(
            f"Erstelle eine Launchkampagne für ein Produkt, dass aus einem Unternehmen mit folgenden Unternehmensinformationen stammt, folgende Zielgruppe bestehend aus folgenden Zielgruppeninformationen hat, und folgende Produktinformationen hat:"
            f"\n\n"
            f"  <unternehmensinformationen>"
            f"\n\n"
            f"      <unternehmensname>{st.session_state['U_Daten']['unternehmensname']}</unternehmensname>"
    ...
    # Zielgruppeninformationen hinzufügen
    prompt_text += "    <zielgruppeninformationen>\n\n"
    if available_data['Z_Personas']:
        for persona in st.session_state['Z_Personas']:
            prompt_text += (
                f"      <user_persona id='{persona['id']}'>\n\n"
    ...
    prompt_text += f"    </zielgruppeninformationen>.\n\n"
    ...
    if available_data['A_Logo']:
        prompt_text += (
            ...
            f"      <auftragsname>Firmenlogo</auftragsname>"
            f"\n\n"
            f"      <extra_informationen>{st.session_state['A_Logo']['extras_text_firma']}</extra_informationen>"
            f"\n\n"
            f"  </auftragsinformationen>."
            f"\n\n"
        )
    ...
    if available_data['A_Produkt']:
        prompt_text += (
            ...
            f"      <tonalität>{st.session_state['A_Produkt']['tonalitaet']}</tonalität>"
            f"\n\n"
            f"      <extra_informationen>{st.session_state['A_Produkt']['extras_text_produkt']}</extra_informationen>"
            f"\n\n"
            f"  </produktinformationen>."
            f"\n\n"
        )
